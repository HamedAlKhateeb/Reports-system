      ...
      <header className="sticky top-0 z-40 w-full border-b border-[#E7E6E2] dark:border-[#2B2B29] bg-[#FAFAF8]/90 dark:bg-[#161615]/90 backdrop-blur-md no-print transition-colors">
        <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-2.5 sm:px-6 lg:px-8">
          ...
          <div className="flex items-center gap-2 sm:gap-6 lg:gap-8">
            <Link href="/reports" className="flex items-center gap-2 font-bold text-[#202020] dark:text-[#F2F2EE] text-base sm:text-lg flex-shrink-0">
              <div className="flex h-8 w-8 sm:h-9 sm:w-9 items-center justify-center rounded-xl bg-[#2E4034] text-white shadow-none">
              ...
              {navItems.map((item) => {
                ...
                return (
                  <Link
                    key={item.href}
                    href={item.href}
                    className={`flex items-center gap-1.5 rounded-xl px-2 sm:px-3 py-1.5 sm:py-2 text-xs font-semibold transition-all ${
                      item.active
          ...
          <div className="flex items-center gap-1.5 sm:gap-2.5 flex-shrink-0">
            ...
            <button
              ...
              className="flex items-center gap-1.5 rounded-xl border border-olive-700/30 bg-[#2E4034] hover:bg-[#24382F] px-2 sm:px-3 py-1.5 text-xs font-semibold text-white transition-all shadow-none"
              title={t('aiAssistant')}
            >
            ...
            <button
              ...
              className="flex items-center gap-1 rounded-xl border border-[#E7E6E2] dark:border-[#2B2B29] bg-white dark:bg-[#20201F] px-2 sm:px-2.5 py-1.5 text-xs font-semibold text-[#202020] dark:text-[#F2F2EE] hover:bg-black/5 dark:hover:bg-white/5 transition-colors"
              title={lang === 'ar' ? 'Switch to English' : 'التبديل إلى العربية'}
            >
              <Globe className="h-3.5 w-3.5 text-olive-700 dark:text-olive-400" />
              <span className="text-[11px] sm:text-xs">{lang === 'ar' ? 'EN' : 'ع'}</span>
            </button>
            ...
            <button
              type="button"
              onClick={() => signOut()}
              className="flex items-center gap-1 rounded-xl p-1.5 sm:p-2 text-[#6B6964] dark:text-[#9E9C96] hover:bg-red-50 dark:hover:bg-red-950/30 hover:text-red-600 transition-colors"
              title={t('logout')}
            >
